...
class Camera implements IObject {
    ...
    Camera(float startX, float startY, float vW, float vH) {
        ...
        maxScale = max(UIState.WORLD_WIDTH / baseW, UIState.WORLD_HEIGHT / baseH);
        update();
    }
    ...
    void handleZoom(float count) {
        viewportScale = constrain(viewportScale + count * UIState.ZOOM_SPEED, UIState.MIN_SCALE, maxScale);
        update();
    }
